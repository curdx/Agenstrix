...
describe("Redactor pipeline: SQLite storage contains no raw secrets", () => {
  test("synthetic secret bytes stored via ptyChunksRepo are redacted before persistence", async () => {
    ...
    const secretPayload = `API_KEY=${FAKE_ANTHROPIC_KEY} GHTOKEN=${FAKE_GITHUB_TOKEN} AWS=${FAKE_AWS_KEY}\n`;
  ...
  test("raw secret bytes (unredacted) stored directly DO appear — confirms test is exercising real data", async () => {
...
describe("Redactor pipeline: bus.publish carries only redacted bytes", () => {
  test("bus subscriber receives redacted bytes — no raw secret in WS-bound events", async () => {
    const { redactChunk } = await import("../../src-bun/pty/redactor");
    ...
    const secretPayload = `tool_output: ${FAKE_ANTHROPIC_KEY} and ${FAKE_GITHUB_TOKEN}\n`;
    const rawChunk = new TextEncoder().encode(secretPayload);
  ...
  test("WS endpoint: connecting to /ws/worker/:id receives redacted bytes from bus", async () => {
    ...
    const wsOpen = new Promise<boolean>((resolve) => {
      ...
      ws.onopen = () => {
        ...
        const secretPayload = `ws_test: ${FAKE_ANTHROPIC_KEY} ${FAKE_AWS_KEY}\n`;
        const rawChunk = new TextEncoder().encode(secretPayload);
        const chunk = redactChunk(rawChunk);

        // Collect frames for a second then close
        const _collectTimeout = setTimeout(() => {
          ws.close(1000);
          resolve(true);
        }, 1000);
...
describe("Env allowlist: ANTHROPIC_API_KEY does not leak to spawn env", () => {
  test("buildSpawnEnv() with ANTHROPIC_API_KEY in process.env returns env WITHOUT the key", async () => {
